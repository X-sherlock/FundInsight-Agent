...
图表解读：多周期收益表现图用于展示多周期基金收益，帮助比较短中长期历史收益窗口的差异，并提示不同窗口不可直接代表未来。

<!-- chart: benchmark_excess_return_1y -->
图表解读：近1年基金、基准与超额收益图并列展示基金收益、基准收益、超额收益，支撑收益分析中的 Data 与 Comparison。

<!-- chart: risk_drawdown_profile -->
图表解读：波动与回撤风险画像图展示最大回撤和波动率，有助于解释收益背后的历史风险代价。

<!-- chart: risk_adjusted_metrics -->
图表解读：风险调整后表现图展示夏普比率、信息比率和 Calmar 比率，辅助观察收益质量而不是形成评分。

<!-- chart: asset_allocation_profile -->
图表解读：资产配置结构图展示资产配置结构，用于解释权益暴露、债券仓位和现金比例对风险特征的影响。

<!-- chart: peer_context -->
图表解读：同类相对位置图展示同类分位和同类均值参照，用于观察同类竞争力分析中的比较依据和数据限制。
...
def test_check_report_accepts_chart_explanation_without_literal_interpretation_word() -> None:
    report = (
        valid_v02_report()
        .replace("图表解读：多周期收益表现图用于展示", "多周期收益表现图展示")
        .replace("图表解读：近1年基金、基准与超额收益图并列展示", "近1年基金、基准与超额收益图展示")
        .replace("图表解读：波动与回撤风险画像图展示", "波动与回撤风险画像图呈现")
        .replace("图表解读：风险调整后表现图展示", "风险调整后表现图呈现")
        .replace("图表解读：资产配置结构图展示", "资产配置结构图呈现")
        .replace("图表解读：同类相对位置图展示", "同类相对位置图呈现")
    )
